...
class Media(threading.Thread):
    ...
    def starttransmit(self):
        self.pipe.send(
            ('starttransmit', ((self.remoteip, self.remoteport), (self.pcapfilename, self.pcapfilter), self.loop)))
        ...
        while running:
                    ...
                    if command == 'opensocket':
                        ...
                        try:
                            ...
                            try:
                                ...
                                sock.close()
                                sock = None
                                exc = Exception("cannot bind UDP socket to {}. errno={}".format(localaddr,
                                                                                                errno.errorcode[
                                                                                                    err.errno]))
                                self.pipe.send(exc)
                            except Exception as exc:
            ...
            if obj is None:
                ...
                if rtp:
                    ...
                    log.info("%s %s:%-5d ---> %s:%-5d RTP(%s)", self, *sock.getsockname(), *remoteaddr,
                             RTP.frombytes(rtp))
...
class RTP:
    ...
    def __str__(self):
        return "PT={} SSRC=0x{:x} Seq={} Time={} + {}bytes".format(self.PT, self.SSRC, self.seq, self.TS,
                                                                   len(self.payload))
    ...
    def _generator(self):
        ...
        for packet in self.udpstream:
            ...
            for k, v in self.pcapfilter.items():
                ...
                if packet.metadata['timestamp'] - inittimestamp < timestamp or packet.metadata[
                    'timestamp'] - inittimestamp > timestamp + datetime.timedelta(seconds=5):
