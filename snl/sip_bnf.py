...
class ParseException(Exception):
    ...
    def __str__(self):
        return "{!r} is not a valid {}. Error at pos={} ({})".format(self.value, self.name, self.pos,
                                                                     self.value[self.pos - 1] if self.pos - 1 < len(
                                                                         self.value) else '')
...
userinfo = user('user') + pp.Optional(pp.Suppress(pp.Literal(':')) + password('password'), None) + pp.Suppress(
    pp.Literal('@'))
...
IPv4address = pp.Combine(
    pp.Word(pp.nums, max=3) + pp.Literal('.') + pp.Word(pp.nums, max=3) + pp.Literal('.') + pp.Word(pp.nums,
                                                                                                    max=3) + pp.Literal(
...
transport_param = pp.CaselessLiteral('transport') + pp.Suppress(pp.Literal('=')) + (
            pp.CaselessLiteral('udp') ^ pp.CaselessLiteral('tcp') ^ pp.CaselessLiteral('sctp') ^ pp.CaselessLiteral(
        'tls') ^ other_transport)
other_user = token
...
method_param = pp.CaselessLiteral('method') + pp.Suppress(pp.Literal('=')) + (
            pp.Literal('INVITE') ^ pp.Literal('ACK') ^ pp.Literal('OPTIONS') ^ pp.Literal('BYE') ^ pp.Literal(
        'CANCEL') ^ pp.Literal('REGISTER') ^ token)
ttl_param = pp.CaselessLiteral('ttl') + pp.Suppress(pp.Literal('=')) + pp.Word(pp.nums, max=3).setParseAction(
...
SIP_SIPS_URI = (pp.CaselessLiteral('sips') | pp.CaselessLiteral('sip'))('scheme') + pp.Suppress(
    pp.Literal(':')) + pp.Optional(userinfo) + hostport + uri_parameters('params') + pp.Optional(headers)('headers')
...
Method = pp.Literal('INVITE') ^ pp.Literal('ACK') ^ pp.Literal('OPTIONS') ^ pp.Literal('BYE') ^ pp.Literal(
    'CANCEL') ^ pp.Literal('REGISTER') ^ token
...
response_auth = pp.CaselessLiteral('rspauth') + pp.Suppress(EQUAL) + pp.Suppress(LDQUOT) + pp.Optional(pp.Word(LHEX),
                                                                                                       '') + pp.Suppress(
    RDQUOT)
...
name_addr = pp.Optional(display_name, None)('display') + pp.Suppress(LAQUOT)('aquot').setParseAction(
    lambda s, loc, toks: [True]) + addr_spec + pp.Suppress(RAQUOT)
...
transport = pp.CaselessLiteral('UDP') ^ pp.CaselessLiteral('TCP') ^ pp.CaselessLiteral('TLS') ^ pp.CaselessLiteral(
    'SCTP') ^ other_transport
